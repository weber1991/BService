...
def login(req):
    if req.method == 'GET':
        ...
        username = req.POST.get('idcard', None)
...
def index(req):
    ...
    try:
        ...
        return render(req, 'dlzp/login_answer.html', {'answer':answer, 'message':message})
    ...
    if zztime:
        if now > zztime.startbm:
            ...
            if username:
                ...
                return render(req, 'dlzp/index.html', {'user': user})
            else:
                return redirect('dlzp:login')
        else:
            answer = '未到开放报名时间'
...
def register(req):
    if req.method == 'GET':
        return render(req, 'dlzp/register.html', {})
    else:
        ...
        if (password1 == password2) and username and idcard:
            ...
            if len(count) == 0:
                ...
                answer = '账号注册成功'
                message = '''<button name="" class="btn btn-success radius size-L" ><a href="/dlzp/login">登 陆</a></button>'''
                return render(req, 'dlzp/register_answer.html', {'answer':answer,'message':message})
            else:
                answer = '账号已被注册'
                message = '''
                <button name="" class="btn btn-success radius size-L" >
					<a href="/dlzp/register/">注 册</a>
				</button>
                '''

                return render(req, 'dlzp/register_answer.html', {'answer': answer,'message':message})
        else:
            answer = '两次密码不一致,请重新注册。'
            message = """<button name="" class="btn btn-success radius size-L" ><a href="/dlzp/register">注 册</a></button>"""
            return render(req, 'dlzp/register_answer.html', {'answer': answer, 'message':message})
...
def reset_password(req):
    if req.method == 'GET':
        return render(req, 'dlzp/reset_password.html', {})
    else:
...
def set_password(req):
    ...
    if req.method == 'GET':
        return render(req, 'dlzp/set_password.html', {'user':user})
    else:
        ...
        if password1 and password2 and (password1==password2):
            ...
            return render(req, 'dlzp/zzb_temp.html', {'title': title, 'content': content})
        else:
            title = '修改失败'
...
def notice_list(req):
    ...
    return render(req, 'dlzp/notice_list.html',
...
def notice_list_all(req):
    ...
    return render(req, 'dlzp/notice_list_all.html',
...
def notice_add(req):
    ...
    if req.method == 'GET':
        #if user:
        #   return redirect('dlzp:login')
        return render(req, 'dlzp/notice_add.html',{})
    else:
        ...
        if title and user and content:
            ...
            return redirect('dlzp:notice_list_all')
        else:
            return redirect('dlzp:notice_add')

def notice_editor(req, id):
    notice = zzNotice.objects.get(id=id)
    if req.method == 'GET':
        return render(req, 'dlzp/notice_editor.html',{'notice':notice})
    else:
        title = req.POST.get('title', None)
        user = req.POST.get('user', None)
        content = req.POST.get('content', None)
        ...
        return render('dlzp:notice_editor', id)

def notice_look(req, id):
    notice = zzNotice.objects.get(id = id)
    return render(req, 'dlzp/notice_look.html', {'notice':notice})
...
def job_add(req):
    if req.method == 'GET':
        return render(req, 'dlzp/job_add.html', {})
    else:
        name = req.POST.get('name', None)
        ...
        if name and daihao and count and education and major and other:
            ...
            return redirect('dlzp:job_add')
        return redirect('dlzp:job_list_all')

# 岗位编辑
def job_editor(req, id):
    job = zzJob.objects.get(id = id)
    if req.method == 'GET':
        return render(req, 'dlzp/job_editor.html',{'job': job})
...
def job_data(req, id):
    job = zzJob.objects.get(id = id)
    return render(req, 'dlzp/job_data.html', {'job':job})

# 岗位查询-对外-对个人
def job_list(req):
    joblist = zzJob.objects.filter(state='发布')
    return render(req, 'dlzp/job_list.html',{'joblist':joblist})

# 岗位查询-对内
def job_list_all(req):
    joblistall = zzJob.objects.all()
    return render(req, 'dlzp/job_list_all.html', {'joblistall': joblistall})
        ...
        if req.method == 'GET':
            ...
            if len(joinjoblist) == 0:
                ...
                try:
                    ...
                    # 年龄不过滤
                    myjoblist = joblist
                    return render(req, 'dlzp/join_job.html', {'joblist': myjoblist, 'userextend': userextend})
                except:
                    title = '无法获取报名信息'
                    content = '请正确填写个人资料。'
                    return render(req, 'dlzp/zzb_temp.html', {'title': title, 'content': content})
            ...
            return render(req, 'dlzp/join_job_suceess.html', {})
        else:
            ...
            if jobid:
                ...
                return redirect('dlzp:join_list')
    else:
        return  redirect('dlzp:login')
...
def join_list(req):
    ...
    if len(joinlist) == 0:
        ...
        return render(req, 'dlzp/zzb_temp.html', {'content':content})
    else:
        joinjob = joinlist.first()
        return render(req, 'dlzp/join_list.html', {'joinjob':joinjob, 'userextend':userextend})

def join_show(req, id):
    joinjob = zzJoinJob.objects.get(id = id)
    userextend = joinjob.user
    return render(req, 'dlzp/join_show.html', {'joinjob':joinjob, 'userextend':userextend})
...
def join_list_all(req):
    ...
    return render(req, 'dlzp/join_list_all.html', {'joinjoblistall': joinjoblistall,
                                                  'pageDict':pageDict,
...
def join_success(req, id):
    ...
    joinjob.state = '审核通过'
    joinjob.save()
    return redirect('dlzp:join_list_all')
...
def join_count(req):
    ...
    return render(req, 'dlzp/join_count.html', {'countlist':countlist})

def join_job_count(req, id):
    job = zzJob.objects.get(id = id)
    joinjoblist = zzJoinJob.objects.filter(job=job)
    return render(req, 'dlzp/join_job_count.html', {'joinjoblist':joinjoblist})
...
def print_joinjob(req, id):
    ...
    try:
        ...
        return render(req, 'dlzp/zzb_bmb.html', {})

    userextend = userextendlist.first()
    if joinjob.user == userextend:
        return render(req, 'dlzp/zzb_bmb.html',{'joinjob':joinjob})
    if user.power > 0:
        return render(req, 'dlzp/zzb_bmb.html', {'joinjob':joinjob})
    return render(req, 'dlzp/zzb_bmb.html', {})
...
def seat_create(req):
    ...
    joinjoblist = zzJoinJob.objects.all()
    if req.method == 'GET':
        return render(req, 'dlzp/seat_create.html', {'joinjobcount':len(joinjoblist)})
    else:
        # 在表单里面验证，而不在后台做
        # 验证数量合不合格
        # 验证输入是否为数字
        ...
        try:
            ...
            pass
        return render(req, 'dlzp/seat_list.html', {'joinjoblist':joinjoblist})
...
def seat_list(req):
    ...
    return render(req, 'dlzp/seat_list.html',
...
def paper_list_all(req):
    ...
    try:
        ...
        joinjoblist = zzJoinJob.objects.all()[(pageDict["nowPage"]-1)*PAGECOUNT:pageDict["nowPage"]*PAGECOUNT]
    return render(req, 'dlzp/paper_list_all.html',
                  {'joinjoblist':joinjoblist,
                   'pageDict':pageDict})
...
def joinjob_excel(req):
    ...
    if joinjoblist:
        ...
        excel_path = "media\\dlzp\\joinjoblist.xls"
...
def seat_excel(req):
    ...
    excel_path = "media\\dlzp\\classlist.xls"
    exist_file = os.path.exists(excel_path)
    if exist_file:
        os.remove(os.path.join(settings.BASE_DIR, excel_path))
    ws.save(os.path.join(settings.BASE_DIR, excel_path))
...
def seat_data_excel(req):
    ...
    excel_path = "media\\dlzp\\seatdatalist.xls"
    exist_file = os.path.exists(excel_path)
    if exist_file:
        os.remove(os.path.join(settings.BASE_DIR, excel_path))
    ws.save(os.path.join(settings.BASE_DIR, excel_path))
...
def chengji_list(req):
    ...
    if req.method == 'GET':
        ...
        #     i.zkzh = zkzh
        #     i.save()
        return render(req, 'dlzp/chengji_list.html',
            ...
            try:
                ...
                for i in range(0, max_row):
                    ...
                    try:
                        ...
                        continue
                return render(req, 'dlzp/chengji_list.html', {'joinjoblist':joinjoblist})
            except:
                return render(req, 'dlzp/chengji_list.html', {})
        return render(req, 'dlzp/chengji_list.html', {})
...
def chengji_look(req, id):
    joinjob = zzJoinJob.objects.get(id = id)
    return render(req, 'dlzp/chengji_data.html', {'joinjob':joinjob})
...
def mydata(req):
    ...
    if len(joinjob) != 0:
        return render(req, 'dlzp/zzb_temp.html', {'title': '告示','content':'您已提交报名，无法修改资料'})
    if req.method == 'GET':
        # 对群众而言，身份证就是登陆账号

        if len(userextendlist) == 0:
            userextend = zzUserExtend.objects.create(user=user, idcard=user.idcard) # ,name=user.name)
            return render(req, 'dlzp/mydata.html', {'userextend': userextend })
        userextend = userextendlist.first()
        return render(req, 'dlzp/mydata.html', {'userextend':userextend})
    else:
...
def time_set(req):
    ...
    if req.method == 'GET':
        return render(req, 'dlzp/time_set.html',
                      {'timeList':timeList})
    else:
        try:
...
def database_set(req):
    ...
    return render(req, 'dlzp/database_set.html', {'zzUserCount':zzUserCount})
